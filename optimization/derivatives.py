...
from models.RegPred.preprocessing import to_one_resolution
...
def dv(reg, a, b, vf, epsilon, name_of_param, vf0, inverse):
    ...
    if name_of_param == 'normalization':
...
def get_derivative_template(data, template, n_steps, vf_all_in_one_resolution,
    ...
    if vf0:
        if inverse:
            ...
            T = 0
        inv_grad_v = np.array([1 / derivative(similarity, template_img, moving_imgs[T],
                                              deformation.backward_dets[-T - 1], vf_all_in_one_resolution[T],
                                              regularizer, 1.)]).reshape(-1, 1)
    else:
        inv_grad_v = np.stack([np.array([1 / derivative(similarity, template_img, moving_imgs[i],
